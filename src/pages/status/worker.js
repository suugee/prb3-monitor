...
const columns = [
  ...
  StringColumn({
    title: 'V',
    mapDataToValue: (data) => (data.session_info?.v / Math.pow(2, 64)).toFixed(2) || '',
  }),
  StringColumn({
    title: 'Ve',
    mapDataToValue: (data) => (data.session_info?.ve / Math.pow(2, 64)).toFixed(2) || 0,
  }),
  StringColumn({
    title: 'totalReward',
    mapDataToValue: (data) => (Number(data.session_info?.stats?.total_reward) / 1000000000000).toFixed(2) || '',
  }),
  StringColumn({
    title: 'startTime',
    mapDataToValue: (data) => {

      let t = data.session_info?.benchmark?.working_start_time
